...
export const useRegistro = () => {
  ...
  const handlerSubmitRegistro = async (e) => {
    ...
    try {
      if (
        userRegistro.password === userRegistro.passwordConfirm &&
        userRegistro.password.length !== 0
      ) {
          ...
          if (errorUsuario === "correcto") {
            if (errorEmail === "correcto") {
              ...
              const res = await fetch(
                "https://lamilagrosa-app.herokuapp.com/api/registro",
                config
              );
        ...
        setTipoError("error");
        setMensaje(
          "Las contraseñas no coinciden o el campo password esta vacio  "
        );
      }
    } catch (error) {}
  };
  ...
  const hendlerInputChangeRegistro = async (e) => {
    ...
    if (e.target.id === "user") {
      ...
      if (
        ...
        const res = await fetch(
          "https://lamilagrosa-app.herokuapp.com/api/getUser",
          config
        );
        const respuesta = await res.json();
    ...
    if (e.target.id === "email") {
      ...
      if (
        ...
        const res = await fetch(
          "https://lamilagrosa-app.herokuapp.com/api/getEmail",
          config
        );
        const respuesta = await res.json();
